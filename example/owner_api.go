...
func RetrieveOutputs(url string, includeSpent, refreshFromNode bool, txID *uint32) (bool, *[]libwallet.OutputCommitMapping, error) {
...
func RetrieveTxs(url string, refreshFromNode bool, txID *uint32, txSlateID *uuid.UUID) (bool, *[]libwallet.TxLogEntry, error) {
	client := RPCHTTPClient{URL: url}
...
func RetrieveSummaryInfo(url string, refreshFromNode bool, minimumConfirmations uint64) (bool, *libwallet.WalletInfo, error) {
	client := RPCHTTPClient{URL: url}
...
// data.
func InitSendTx(url string, initTxArgs libwallet.InitTxArgs) (*libwallet.Slate, error) {
...
func TxLockOutputs(url string, slate *libwallet.Slate, participantID uint) error {
	client := RPCHTTPClient{URL: url}
...
func FinalizeTx(url string, slateIn libwallet.Slate) (*libwallet.Slate, error) {
	client := RPCHTTPClient{URL: url}
...
func PostTx(url string, tx core.Transaction, fluff bool) error {
	client := RPCHTTPClient{URL: url}
...
func CancelTx(url string, txID *uint32, txSlateID *uuid.UUID) error {
	client := RPCHTTPClient{URL: url}
...
func NodeHeight(url string) (*libwallet.NodeHeightResult, error) {
	client := RPCHTTPClient{URL: url}
